...
// Small canvas used for the mobile/Safari floating-timer fallback (see below).
const PIP_WIDTH = 320;
...
function FullScreenTimer({ timer, onClose }) {
  ...
  const isComplete = hasLiveContextTimer
    ...
    : !isSeoTimer && Boolean(currentTimer?.status === 'completed' || remaining <= 0);
  const time = useMemo(() => formatTime(remaining), [remaining]);
  const progress = currentTimer?.duration ? ((currentTimer.duration - remaining) / currentTimer.duration) * 100 : 0;
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      tickAudioRef.current.addEventListener('error', () => {
        console.warn('[FullScreenTimer] /sounds/tick.mp3 could not be loaded — using a synthesized tick instead.');
      });
    }
    return () => {
      if (tickAudioRef.current) {
  ...
  // Play tick sound from public/sounds/tick.mp3
  const playTickSound = () => {
  ...
  // ---------------- AMBIENT / FOCUS SOUND (fixed) ----------------
  // Generated locally with Web Audio — no external sound files needed, so
  // "Rain", "Lofi Beats" and "Ocean" always work.
  ...
  // -------------- END AMBIENT / FOCUS SOUND --------------

  // ---------------- FLOATING TIMER (Document PiP + mobile fallback) ----------------
  // Chrome/Edge desktop: uses the rich Document Picture-in-Picture window (interactive,
  // has its own buttons) — unchanged from before.
  // Everywhere else (Android Chrome, Safari desktop/iOS): falls back to a canvas-driven
  // video stream fed into the standard video Picture-in-Picture API, so floating still
  // works on mobile and PC even without documentPictureInPicture support.
  ...
  const drawPipFrame = () => {
    ...
    const timeText = showHours ? `${time.hours}:${time.minutes}:${time.seconds}` : `${time.minutes}:${time.seconds}`;
    ...
    ctx.fillRect(0, h - barHeight, w * Math.min(1, Math.max(0, progress / 100)), barHeight);
  ...
  const openVideoPip = async () => {
    ...
    try {
      ...
      window.alert('Floating Timer is not supported in this browser.');
    }
  };
  ...
  const openFloatingTimer = async () => {
    ...
    // Mobile / Safari fallback: no documentPictureInPicture available.
    if (videoPipSupported) {
      openVideoPip();
      return;
    }

    window.alert('Floating Timer needs Chrome, Edge, or Safari (a recent version) to work.');
  };
  ...
  // Audible one-second tick while the timer is running
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-gradient-to-br from-[#070707] via-[#180a29] to-[#070707] text-white flex flex-col">
      {/* Hidden canvas + video powering the mobile/Safari floating-timer fallback. */}
      <canvas
        ref={pipCanvasRef}
        width={PIP_WIDTH}
        height={PIP_HEIGHT}
      ...
      {/* Centered Floating Bar - Extra Margin Top (Neeche Shifted) */}
<div className="relative z-10 mx-auto mt-10 sm:mt-16 w-full max-w-xl flex items-center justify-between gap-3 p-3 sm:p-4 bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl">
  {/* Timer Details */}
  <div className="min-w-0 flex-1">
    <h1 className="font-bold text-sm sm:text-base text-white truncate max-w-[140px] sm:max-w-[220px]">
      {currentTimer.name || 'Timer'}
    </h1>
    <p className="text-[10px] sm:text-xs text-white/60 capitalize mt-0.5">
      {currentTimer.type || 'custom'} · {isPaused ? 'Paused' : isComplete ? 'Completed' : 'Running'}
    </p>
  </div>
  ...
  <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
    <button
      ...
      className="p-2 sm:px-3 sm:py-2 rounded-xl bg-white/10 hover:bg-white/20 active:scale-95 text-xs sm:text-sm transition-all"
      aria-label="Toggle sound"
    ...
    <button
      ...
      className={`p-2 sm:px-3 sm:py-2 rounded-xl active:scale-95 text-xs sm:text-sm transition-all ${
        isFloating ? 'bg-purple-500/30 text-purple-300' : 'bg-white/10 hover:bg-white/20'
      } ${!floatingSupported ? 'opacity-40 cursor-not-allowed active:scale-100' : ''}`}
      aria-label="Floating timer"
      title={floatingSupported ? 'Keep timer floating' : 'Use Chrome, Edge, or Safari for Floating Timer'}
    >
      🪟
    </button>
    ...
    <button
      onClick={handleClose}
      className="p-2 sm:px-3 sm:py-2 rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30 active:scale-95 text-xs sm:text-sm font-semibold transition-all"
      aria-label="Close timer"
      title="Close"
    >
      ✕
...
{/* Extra Feature: Ambient Sound & Quick Audio Bar (Right Below Control Bar) */}
<div className="relative z-10 mx-auto mt-3 w-full max-w-xl flex items-center justify-between gap-2 p-2.5 px-4 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl text-xs sm:text-sm text-white/80">
  <div className="flex items-center gap-2">
    <span>🎧 Focus Sound:</span>
    <select
      value={ambient}
      onChange={(e) => setAmbient(e.target.value)}
      className="bg-black/50 text-white border border-white/20 rounded-lg px-2 py-1 text-xs focus:outline-none focus:border-white/50 cursor-pointer"
    >
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-4 sm:py-6">
        {showComplete ? (
          <div className="text-center mb-4 sm:mb-6">
            <div className="text-6xl sm:text-9xl mb-3 sm:mb-4">🎉</div>
            <h2 className="text-2xl sm:text-5xl font-bold">Timer Complete!</h2>
            <p className="text-purple-300 mt-1 sm:mt-2 text-sm sm:text-base">{currentTimer.name}</p>
          </div>
        ) : (
          <>
            <div className="text-[clamp(4rem,16vw,11rem)] leading-none font-mono font-bold tracking-tight text-center tabular-nums">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-4 sm:mt-6">
          {!showComplete && (
            <button onClick={togglePause} className="px-5 py-2.5 sm:px-7 sm:py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 font-bold text-sm sm:text-base">
              {isPaused ? '▶ Resume' : '⏸ Pause'}
            </button>
          )}
          <button onClick={handleReset} className="px-5 py-2.5 sm:px-7 sm:py-3 rounded-xl bg-white/10 hover:bg-white/15 font-semibold text-sm sm:text-base">🔄 Reset</button>
          <button onClick={handleClose} className="px-5 py-2.5 sm:px-7 sm:py-3 rounded-xl bg-white/10 hover:bg-white/15 font-semibold text-sm sm:text-base">✕ Close</button>
        </div>

        <div className="mt-5 sm:mt-7 flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs text-white/40">
          <label htmlFor="fullscreen-volume" className="whitespace-nowrap">Volume</label>
          <input id="fullscreen-volume" type="range" min="0" max="100" value={volume} onChange={(event) => setVolume(Number(event.target.value))} disabled={!soundEnabled} className="w-20 sm:w-32" />
          <span className="min-w-[30px]">{volume}%</span>
        </div>
      </main>
